...
class MS_MLP_Conv(nn.Module):
    def __init__(
        ...
        if spike_mode == "lif":
            ...
            self.mlp1_lif = MultiStepParametricLIFNode(
                init_tau=2.0, detach_reset=True, backend="cupy"
            )

        self.mlp2_conv = SpikingConv2d(
    ...
    def forward(self, x, hook=None):
        ...
        x = self.mlp1_lif(x)  # Output 5D Spikes
        if hook is not None:
            hook[self._get_name() + str(self.layer) + "_mlp1_lif"] = x.detach()
        ...
        x = self.mlp2_lif(x)  # Output 5D Spikes
        if hook is not None:
            hook[self._get_name() + str(self.layer) + "_mlp2_lif"] = x.detach()
        ...
        x = x + identity
        return x, hook
    def prune_parameters(self, threshold, optimizer,max_prune_rate=0.1):

        probs = self.mlp2_conv.get_average_mask_probs()
        ...
        if kept < total:
            ...
            prune_batchnorm(self.mlp1_bn, keep_indices, optimizer=optimizer)
