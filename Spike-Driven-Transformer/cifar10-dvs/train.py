...
parser.add_argument(
    "--output",
    default="spike-driven-transformer/CIFAR10DVS/output/train",
    type=str,
    metavar="PATH",
...
parser.add_argument('--gumbel-temp-start', type=float, default=5.0,
...
parser.add_argument('--pruning-interval', type=int, default=25, metavar='N',
...
parser.add_argument('--pruning-end-offset', type=int, default=40, metavar='N',
...
def main():
    ...
    if args.rank == 0:
        ...
        if output_dir is None:
             output_dir = get_outdir(args.output if args.output else './output/train', "default_exp")

        _logger.handlers = []
        _logger.setLevel(logging.INFO)

        log_file_path = os.path.join(output_dir, 'SPIKE-DRIVEN-TRANSFORMER.log')
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            if epoch == fine_tuning_start_epoch:
                ...
                if args.distributed and args.sync_bn:
                    if has_apex and use_amp != "native":
                        real_model = convert_syncbn_model(real_model)
                    else:
                        real_model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(real_model)
            ...
            if is_pruning_epoch:
                ...
                if hasattr(real_model, 'prune_model'):
                    ...
                    for m in real_model.modules():
                        if isinstance(m, SpikingConv2d):
                            m.mask_accumulator = None
                            m.accumulation_steps = 0
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            if args.TET:
                ...
                loss = loss_fn(output, target)+ loss_reg
    ...
    return OrderedDict([
        ("loss", losses_m.avg),
        ("reg_loss", gating_losses_m.avg),
        ("temp", temp_m.avg if temp_scheduler else 0.0)
    ])
