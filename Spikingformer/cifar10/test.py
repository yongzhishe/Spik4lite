...
parser.add_argument('--resume',
                    default='spikingformer/cifar10/output/oblation1/lambda-energy_0.01/model_best.pth.tar',
...
parser.add_argument('--output', default='spikingformer/cifar10/output/same_acc_lambda0.01_test',
                    type=str,
                    metavar='PATH',
...
def infer_structure_config_from_state_dict(state_dict):
    ...
    if pe_cfg:
        ...
        _logger.info(f"[Inference] Automatically inferred Tokenizer dims from weights: {pe_cfg}")
    ...
    if blocks_cfg:
        ...
        _logger.info(f"[Inference] Automatically inferred MLP hidden dims for {len(blocks_cfg)} blocks.")
    return cfg
...
def get_pruned_config(checkpoint_path):
    ...
    try:
        _logger.info(f"Peeking at checkpoint: {checkpoint_path}")

        ckpt = torch.load(checkpoint_path, map_location='cpu')
        epoch = ckpt.get('epoch', 0) if isinstance(ckpt, dict) else 0

        if isinstance(ckpt, dict) and 'structure_config' in ckpt:
            _logger.info("Found explicit 'structure_config' in checkpoint.")
            return ckpt['structure_config'], epoch

        _logger.info("No explicit config found. Inferring structure from weight shapes...")
        state_dict = ckpt
        ...
        _logger.warning(f"Error reading checkpoint for config: {e}")
        return None, 0
...
def load_weights_ignoring_gating(model, checkpoint_path):
    ...
    _logger.info(f"Loading weights from {checkpoint_path} (Filtering gating keys)...")
    ...
    _logger.info("Weights loaded successfully.")
...
def main():
    ...
    if args.resume:
        ...
        load_weights_ignoring_gating(model, args.resume)
    ...
    try:
        ...
        _logger.info(">>> Validating Pruned Model (Inference Mode)")
        eval_metrics = validate(model, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast)
        print('Pruned model metrics:', eval_metrics)
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            with amp_autocast():
                # with torch.profiler.profile(use_cuda=True, record_shapes=True, profile_memory=True,
                #             on_trace_ready=torch.profiler.tensorboard_trace_handler('./log'), with_stack=True) as prof:
    ...
    _logger.info(f"Test metrics: {metrics_str}")
