...
parser.add_argument('--output', default='spikingformer/cifar10/output/oblation_infer', type=str, metavar='PATH',
...
parser.add_argument('--log-output-dir',
                    default='spikingformer/cifar10/output/energy_infer/', type=str,
...
def infer_structure_config_from_state_dict(state_dict):
    ...
    for i in range(5):
        ...
        if f'module.{key}' in state_dict: key = f'module.{key}'
...
def get_config_from_checkpoint(checkpoint_path):
    if not os.path.exists(checkpoint_path):
        _logger.warning(f"Checkpoint file not found: {checkpoint_path}")
        return None

    _logger.info(f"Peeking into checkpoint: {checkpoint_path}")
    try:
    ...
    if isinstance(checkpoint, dict):
        if 'structure_config' in checkpoint:
            ...
            return checkpoint['structure_config']
    ...
    _logger.info("Inferring structure from weight shapes...")
    cfg = infer_structure_config_from_state_dict(state_dict)
...
def load_weights_clean(model, checkpoint_path):
    ...
    try:
        ...
        _logger.info("Weights loaded successfully (Strict Mode).")
    except RuntimeError as e:
        _logger.warning(f"Strict loading failed: {e}")
...
def main():
    ...
    if args.resume:
        chk_name = os.path.basename(args.resume)
    else:
        chk_name = "no_checkpoint"

    if not os.path.exists(args.log_output_dir):
        os.makedirs(args.log_output_dir)

    date_str = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_filename = f"{chk_name}_{date_str}.log"
    log_path = os.path.join(args.log_output_dir, log_filename)

    sys.stdout = DualOutput(log_path)
    sys.stderr = DualOutput(log_path)
    ...
    if args.resume:
        pruned_structure_cfg = get_config_from_checkpoint(args.resume)
    ...
    model = create_model(
        ...
        pruned_structure_cfg=pruned_structure_cfg
    )
    ...
    if args.resume:
        ...
        load_weights_clean(model, args.resume)
    ...
    Nops, Nparams = get_model_complexity_info(model, (3, 224, 224), loader_eval, as_strings=True,
