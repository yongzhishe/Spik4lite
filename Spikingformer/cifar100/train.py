...
from Spik4lite import EnergyPenaltyTerm, SparsityTargetTerm, GumbelTemperatureScheduler, SpikingConv2d
...
parser.add_argument('-c', '--config', default='Spikingformer/cifar100/cifar100.yml', type=str, metavar='FILE',
...
parser.add_argument('-data-dir', metavar='DIR', default="CIFAR100/",
...
parser.add_argument('--output', default='spikingformer/cifar100/output/train',
                    type=str,
                    metavar='PATH',
...
parser.add_argument('--target-sparsity', type=float, default=   None,
                    help='Target drop rate (0.0-1.0). If set, model will try to prune strictly X% of channels globally. '
                         'Example: 0.5 means aiming for 50% drop rate.')
parser.add_argument('--gumbel-temp-start', type=float, default=3.0,
                    help='initial Gumbel temperature (default: 1.0)')
parser.add_argument('--gumbel-temp-end', type=float, default=1.0,
                    help='final Gumbel temperature (default: 0.5)')

parser.add_argument('--pruning-interval', type=int, default=60, metavar='N',
...
parser.add_argument('--pruning-threshold', type=float, default=0.2, metavar='VAL',
                    help='Threshold for pruning channels based on accumulated mask probability (default: 0.5)')
parser.add_argument('--pruning-end-offset', type=int, default=80, metavar='N',
                    help='Stop pruning this many epochs before the end (fine-tuning phase, default: 20)')
parser.add_argument('--pe-max-prune-rate', type=float, default=0.1, metavar='VAL',
                    help='Maximum pruning rate per step for Patch Embedding layers (default: 0.4)')
parser.add_argument('--mlp-max-prune-rate', type=float, default=0.15, metavar='VAL',
...
def main():
    ...
    if args.resume:
        ...
        if not args.no_resume_opt:
            if loaded_optimizer_state and optimizer is not None:
                try:
                    optimizer.load_state_dict(loaded_optimizer_state)
                    if args.local_rank == 0: _logger.info('Restoring optimizer state from checkpoint.')
                except Exception as e:
                    if args.local_rank == 0: _logger.warning(
                        f'Could not restore optimizer state (expected if pruned): {e}')

            if loaded_scaler_state and loss_scaler is not None:
                loss_scaler.load_state_dict(loaded_scaler_state)
                if args.local_rank == 0: _logger.info('Restoring AMP loss scaler state from checkpoint.')
    ...
    if args.target_sparsity is not None:
        if args.local_rank == 0:
            _logger.info(f"Using Target Mode: Enforcing {args.target_sparsity} sparsity (MSE Loss).")

        regularizer_criterion = SparsityTargetTerm(
            model,
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            if args.target_sparsity is not None:
                max_l = 5.0
                warmup_epochs = int(0.2 * args.epochs)
                current_l = max_l * min(1.0, epoch / warmup_epochs)
                regularizer_criterion.set_lambda(current_l)
                if args.local_rank == 0:
                    _logger.info(f"Sparsity Lambda updated to: {current_l:.4f}")
            ...
            if is_pruning_epoch:
                ...
                if hasattr(real_model, 'prune_model'):
                    ...
                    if args.local_rank == 0 and pruning_csv_path:
                        with open(pruning_csv_path, mode='a', newline='') as f:
                            writer = csv.writer(f)
                            for stat in pruning_stats:
                                writer.writerow(
                                    [epoch, stat['layer'], stat['total'], stat['kept'], f"{stat['ratio']:.4f}"])
                                _logger.info(
                                    f"  Pruned {stat['layer']}: {stat['total']} -> {stat['kept']} (Ratio: {stat['ratio']:.2%})")
